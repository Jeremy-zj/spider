...
class GetUser(threading.Thread):
    ...
    # 获取新问答html
    def get_index_newPage(self):
        index_url = 'https://www.zhihu.com/question/waiting?type=new'
        try:
            index_html = self.session.get(index_url, headers=self.headers, timeout=35)
        except Exception as err:
            # 出现异常重试
    ...
    # 获取问题列表，存入redis
    def get_index_newPage_question(self):
        index_html = self.get_index_newPage()
        if not index_html:
            return
        BS = BeautifulSoup(index_html, "html.parser")
        # 获取用户的a标签
        question_a = BS.find_all("a", class_="ListQuestionItem-title QuestionWaiting-questionItemTitle")  
        for a in question_a:
            if a:
                href = a.get('href')
                self.add_wait_question(href[(href.rindex('/')) + 1:])
